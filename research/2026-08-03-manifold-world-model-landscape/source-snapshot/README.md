<p align="center"><sub>RESEARCH REPOSITORY</sub></p>

<!-- publication-status: public -->

<h1 align="center">Manifold 海外世界模型竞争格局</h1>

<p align="center"><strong>2026-07-14 研究项目复盘：从三份 Deep Research，到一条可审计、可反驳、可复现的研究决策链</strong></p>

<p align="center"><strong>Research design &amp; decision ownership:</strong> <a href="https://github.com/Madarame87">@Madarame87</a></p>

<p align="center">
  <a href="#design-ownership">我的设计</a>&nbsp;&nbsp;·&nbsp;&nbsp;
  <a href="#research-origin">研究起点</a>&nbsp;&nbsp;·&nbsp;&nbsp;
  <a href="#decision-trail">决策链</a>&nbsp;&nbsp;·&nbsp;&nbsp;
  <a href="#postmortem">经验与改进</a>&nbsp;&nbsp;·&nbsp;&nbsp;
  <a href="#ai-value">AI 增益</a>&nbsp;&nbsp;·&nbsp;&nbsp;
  <a href="#conclusions">最终结论</a>&nbsp;&nbsp;·&nbsp;&nbsp;
  <a href="#audit">复核路径</a>
</p>
...
>
> **脱敏状态：已完成。** 公开发布前已移除本机绝对路径、私人联系邮箱、工作区路径和冗余打包文件中的机器元数据，并完成文本、Office 文件与凭据特征扫描。具体处理范围和可复核结果见 [`公开发布审计`](docs/reproducibility/public-release-audit.md)。
>
> **公开平台边界：** 本仓库为 Public，任何人均可能查看、索引或通过 GitHub 平台功能 Fork。除平台功能允许的查看与 Fork 外，未经研究设计者书面许可，不得复制、转载、改编或用于商业用途。

这个仓库首先是一份留给未来复查的工作记录。最终 PPT 浓缩结论，仓库保存形成这些结论的过程与证据；两者是同一项研究的不同层次，而不是互相补救。

如果只读这个仓库的一个文件，就读这份 README。它记录：**问题为什么这样拆解、每一步解决什么风险、AI 与 Agent 在哪里产生真实增益、哪些做法值得复用，以及下一次如何进一步改进。**

最终研究对象：**World Labs、Odyssey、Runway、Decart、NVIDIA Cosmos 3**。

**5 分钟阅读路径：** [`研究设计`](#design-ownership) → [`完整决策链`](#decision-trail) → [`经验与改进`](#postmortem) → [`AI 的真实增益`](#ai-value) → [`最终结论`](#conclusions)。项目形成的 10 页结论版本见 [`最终 PPTX`](archive/manifold-world-model-landscape/10_deck/manifold_world_model_landscape_v1_4.pptx)。
...
[`DR-01 Manifold 公开比较锚点.md`](<inputs/original/DR-01 Manifold 公开比较锚点.md>) 汇总了 Manifold 的公开公司口径、产品项目、技术能力、融资与商业线索，并提出了一个比较锚点框架。
...
[`DR-02 非中国玩家发现池与三层漏斗.md`](<inputs/original/DR-02 非中国玩家发现池与三层漏斗.md>) 给出了发现池、边界案例、技术路线、应用任务、覆盖审计和初步候选建议。
...
[`DR-03 大厂项目 平台替代与垂直替代路线.md`](<inputs/original/DR-03 大厂项目 平台替代与垂直替代路线.md>) 梳理了 NVIDIA、Google、Meta 等大厂项目，以及机器人、自动驾驶、仿真和内部组件等替代路线。
...
另外两份 v2.0 治理文件定义了硬边界、审批检查点和证据标准：[`项目执行计划`](inputs/original/Manifold_World_Model_Research_Project_Plan_v2.0_CN.md) 与 [`启动 Prompt`](inputs/original/Manifold_ChatGPT_Work_Launch_Prompt_v2.0_CN.md)。三份 DR **只作线索**；决定范围、资格和结论的关键主张必须重新打开原始来源。

<a id="decision-trail"></a>
...
**具体做法：** 主 Agent 先建立 [`project_state.json`](archive/manifold-world-model-landscape/00_brief/project_state.json)、[`decision_log.md`](archive/manifold-world-model-landscape/00_brief/decision_log.md) 和 [`input_manifest.csv`](archive/manifold-world-model-landscape/01_inputs/input_manifest.csv)。每个输入记录来源路径、工作副本、字节数、时间和 SHA256；每个阶段由状态文件控制是否允许启动。
...
对应产物是 [`eligibility_register.csv`](archive/manifold-world-model-landscape/03_universe/eligibility_register.csv) 和三份输入审计记录。
...
**阶段结果：** 检查点 1 最终批准的是**范围、规则、Manifold 公开锚点和 10 对象验证队列**，并没有提前选五席。审批包、规则补丁和 QA 保存在 [`05_candidates/`](archive/manifold-world-model-landscape/05_candidates/) 中。
...
检查点 2 将结果固定为 **5 个重点对象、4 个观察对象、1 个早期信号**。Meta V-JEPA family 被修正为受限平台替代的 `core watch`，而不是因为“没有托管 API”就判为无战略关系。详见 [`checkpoint_2_recommendation_pack.md`](archive/manifold-world-model-landscape/05_candidates/checkpoint_2_recommendation_pack.md) 与 [`checkpoint_2_v1_0a_control_patch.md`](archive/manifold-world-model-landscape/05_candidates/checkpoint_2_v1_0a_control_patch.md)。
...
- 形成 [`5 份对象 Dossier`](docs/dossiers/)；普通读者只会看到五个清楚命名的对象页面，方法和 QA 与对象研究分开存放；
- 执行 **58 次定向支持/反证查询**；
...
**具体做法：** [`competitive_matrix.csv`](archive/manifold-world-model-landscape/07_synthesis/competitive_matrix.csv) 只比较四个维度：技术证据、外部可获得性、商业采用上限、对 Manifold 的条件性意义。之后从三条路线攻击结果：
...
详见 [`red_team_report.md`](archive/manifold-world-model-landscape/09_red_team/red_team_report.md)、[`cross_comparison.md`](archive/manifold-world-model-landscape/07_synthesis/cross_comparison.md) 和 [`monitoring_matrix.csv`](archive/manifold-world-model-landscape/07_synthesis/monitoring_matrix.csv)。
...
**这一步的结果：** 形成 **63 条唯一页面证据映射**，覆盖 10 个内容单元，并通过 CSV/XLSX 逐单元一致性和冻结哈希复核。详见 [`slide_evidence_map.csv`](archive/manifold-world-model-landscape/08_content/slide_evidence_map.csv) 与 [`content_lock_v1_1a_frozen_manifest.md`](archive/manifold-world-model-landscape/08_content/content_lock_v1_1a_frozen_manifest.md)。
...
它经历了 4 个 PPTX 版本和多轮视觉整改，最终 10 页、252 个文本框通过机械与人工 QA。最终 PPTX 作为结论表达保存在 [`10_deck/`](archive/manifold-world-model-landscape/10_deck/)，相关生成代码、渲染记录和 QA 同时保留，便于复查内容如何被转化为页面。

## 11 · 第九步：把一次性项目整理为 Research Repository

项目结束时，输入、研究记录、证据、代码和 QA 分散在不同工作目录中。这一步把它们整理成可长期维护、可以反向检查自身判断的系统：

- 原始输入单独保存在 [`inputs/original/`](inputs/original/)；
- 原工作区 333 个阶段文件经过公开发布审计后，在 [`archive/manifold-world-model-landscape/`](archive/manifold-world-model-landscape/) 保留 331 个公开安全文件；两个带机器本地路径元数据、且不含独有内容的派生 ZIP 字节包不公开，其原始 SHA256 和 16 个成员文件仍完整保留；
- Storyline、方法、Research Log、Agent 和结果被重组到 [`docs/`](docs/)；
- Agent 创建、实际启动、输入输出角色和工具事件保存在 [`provenance/`](provenance/)；
- [`scripts/audit/verify_repository.ps1`](scripts/audit/verify_repository.ps1) 可机械复核输入、来源、Claims、Evidence Map、Agent 和关键哈希；
...
整个会话记录了 33 次 Agent 创建请求，其中 30 个实际启动。这个数字本身并不代表质量；有意义的是每个 Agent 是否解决了一个边界清楚的问题，以及结果是否进入可验证的 Claim、Source 或 QA。完整角色、输入和输出见 [`agent_register.csv`](provenance/agent_register.csv)、[`Agent I/O`](docs/agents/agent-io.md) 和 [`Agent 编排方式`](docs/agents/orchestration.md)。
...
完整结论和五席证据边界见 [`最终结论`](docs/overview/final-conclusions.md)、[`固定五席`](docs/results/final-five.md) 与 [`观察名单`](docs/results/watchlist.md)。
...
2. 查看 [`input_manifest.csv`](archive/manifold-world-model-landscape/01_inputs/input_manifest.csv) 和 [`eligibility_register.csv`](archive/manifold-world-model-landscape/03_universe/eligibility_register.csv)，确认输入如何变成对象登记；
3. 查看 [`sources.csv`](archive/manifold-world-model-landscape/04_evidence/sources.csv)、[`来源卡目录`](archive/manifold-world-model-landscape/04_evidence/sources/) 和 [`evidence_ledger.csv`](archive/manifold-world-model-landscape/04_evidence/evidence_ledger.csv)，抽查 Claim 是否回到原始来源；
4. 阅读 [`五份 Dossier`](docs/dossiers/) 和 [`横向比较`](archive/manifold-world-model-landscape/07_synthesis/cross_comparison.md)；
5. 对照 [`Red Team 报告`](archive/manifold-world-model-landscape/09_red_team/red_team_report.md) 查看哪些结论被降级、修正或保留；
6. 运行机械审计：
